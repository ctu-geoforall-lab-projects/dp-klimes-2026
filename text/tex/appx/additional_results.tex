...
\node[rotate=90, font=\scriptsize] at (-2.2,2.25) {True label};
    ...
    \end{subfigure}
    \caption[Confusion matrices for Sentinel-2 models on the Sentinel-2 test set.]{Confusion matrices for the Sentinel-2 models on the Sentinel-2 test set. Values are normalized by row.}
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption[Additional prediction examples for Sentinel-2 model with dynamic Z-score normalization.]{Additional prediction examples for the Sentinel-2 model with dynamic Z-score normalization on the Sentinel-2 test set.}
